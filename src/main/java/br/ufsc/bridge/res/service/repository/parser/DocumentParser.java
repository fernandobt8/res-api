...
public class DocumentParser {
	...
	public void parser(ProvideAndRegisterDocumentSetRequestType provideRegister, RepositorySaveDocumentDTO dto) {
		...
		.buildExtrinsicObject()
			...
			//dados paciente
			.buildClassification()
				.classificationScheme("urn:uuid:ab9dd76d-024a-4f8f-b9a5-4e0be6911878")
				.id("cl" + this.atomicLong.incrementAndGet())
				.nodeRepresentation("")
				.buildSlot()
					.name("nome").value(dto.getNomePaciente())
				.addSlot()
					.name("dataNascimento").value(RDateUtil.dateToISOXDSb(dto.getDataNascimentoPaciente()))
				.addSlot()
					.name("sexo").value(dto.getSexoPaciente())
				.addSlot()
					.name("uf").value(dto.getUfPaciente())
				.addSlot()
					.name("cidade").value(dto.getCidadePaciente())
				.addSlot()
					.name("bairro").value(dto.getBairroPaciente())
				.addSlot()
					.name("logradouro").value(dto.getLogradouroPaciente())
				.addSlot()
					.name("numeroCasa").value(dto.getNumeroCasaPaciente())
				.addSlotEnd()
			.addClassificationEnd()
			//classCode
			.buildClassification()
